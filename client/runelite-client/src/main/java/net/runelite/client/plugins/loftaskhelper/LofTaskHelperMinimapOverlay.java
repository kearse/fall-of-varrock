/*
 * Fall of Varrock — Task Helper minimap overlay.
 *
 * When the hunting ground sits inside the minimap's view, a small arrow bobs over its spot; when
 * it's beyond the edge, an arrow at the minimap's rim points the way (the classic Quest Helper
 * behaviour — see LofArrow in lofquests for the adapted math).
 */
...
class LofTaskHelperMinimapOverlay extends Overlay
{
	...
	@Inject
	LofTaskHelperMinimapOverlay(Client client, LofTaskHelperPlugin plugin, LofTaskHelperConfig config)
	{
		this.client = client;
		this.plugin = plugin;
		this.config = config;
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		// In view: a small arrow bobbing over the hunting ground's minimap spot.
		...
		// Out of view: an arrow at the minimap's rim pointing toward the hunting ground.
