...
| **Companion stable slots** ⬜ | Own 3 companions, **deploy one at a time** (hard rule) | Variety/convenience, zero combat power. **Unblocked 2026-09-02:** "owned" and "fielded" are separate counts (`CompanionRegistry.rosterSize` vs `count`; a companion can be *dismissed* and summoned back in a swap) and the hard *deploy-one* rule is locked at the engine (`CompanionRegistry.ACTIVE_MAX = 1`, enforced every tick). `Title.roster` sets the ROSTER by rank (1/2/3), never the fielded count — so a sold slot would only ever be roster variety, never army size. Not built; the seam is `rosterCap` |
