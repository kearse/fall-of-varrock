...
| 10 | `pvm/10-retire-boss-tickets` | Boss Tickets retired from every boss/minigame; Champion's cape / Divine halo at 100 / 500 boss kills | — | ✅ PR #337 MERGED |
| 11 | `pvm/12-guardians-of-the-rift` → `claude/restore-gotr-395791` | Guardians of the Rift (operator pick 2026-09-03): temple lobby/round loop, mining → essence → altar portals → Great Guardian, barrier line vs abyssal creatures, points, Rewards Guardian + Temple Supplies (Team 2 approved) — spec `docs/pvm/minigames-b-spec.md` | OSRS wiki | ✅ **RESTORED to main 2026-09-12.** PR #340 merged into the stacked `pvm/12-…` branch and never reached `main`; re-landed from 40709593 (+ the auditor follow-up 01e11765) with the shared rows deduplicated against #355 |
| 12 | `pvm/13-blast-furnace` → PR #355 | Blast Furnace (operator pick): belt → machine → dispenser, coal bag, ice gloves, OSRS coffer 72k/h (Team 2), no Ordan | OSRS wiki | ✅ **RESTORED to main 2026-09-12** by The Guns of Asgarnia (#355): same stacked-branch loss as GotR; dispenser bound on child ids, Foreman on NpcTalk, `feeWaivers` / `barListeners` seams |
| — | deferred | Tempoross, Giants' Foundry, Barbarian Assault (operator: not now) | OSRS wiki | ⬜ |

> **Stacked-PR lesson (2026-09-12).** A PR whose base is another PR's branch shows MERGED when it merges into
> that branch — the code reaches `main` only if the base PR later merges too. #340 and #341 were both lost
> this way (their base `pvm/12-…` was never merged). Verify with `git merge-base --is-ancestor <merge-sha> main`.
