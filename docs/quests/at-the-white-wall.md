...
| Inspect the front (3 observations, any order) | The checkpoint (2957-2973 × 3395-3400) · the gate road inside the wall (2961-2970 × 3383-3391) · the field beyond the fence (2956-2974 × 3406-3414) | `Objective.Predicate` on the framework poll; per-observation counters in the quest state | step `front` — `AtTheWhiteWall.observe` narrates each once, clears when all three are seen. The guidance marker **moves within the step** (`QuestStep.anchorFor` → `nextObservation`): it sits on the next place still unread, never on one already ticked off — a fixed anchor left the arrow on a tile with nothing behind it, which reads as a broken marker (reported 2026-09-18). The chat names what is left instead of counting it |
