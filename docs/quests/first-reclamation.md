...
| Establish the Southern Watch: **plant the standard** | The Castle Wars **Saradomin Standard** (option **Capture** — a banner on its stand) spawned at 3227,3372, north of the centre stone. The loc **and its shape** are **resolved at boot** from `SouthernWatch.STANDARD_CANDIDATES`, not hard-coded: a varbit-gated candidate is followed through its `transforms` to a concrete id, and the shape comes from the loc's own `objectTypes` (models are registered per shape — spawn one in a shape it has no model for and the client draws nothing, leaving an invisible, unclickable object) | `onObjOption` (bound defensively from the cache actions of the resolved loc) | `SouthernWatchPlugin.resolveStandard` → `spawnStandard`/`bindStandard` → `SouthernWatch.onCapture` hook → `FirstReclamation.raiseStandard` → `satisfy(establish)` |
| …if the cache offers no clickable banner at all | the ground at 3227,3372 | world timer, only while `!SouthernWatch.standardClickable` or nothing is spawned on the tile | `FirstReclamationPlugin.groundFallback` → `raiseStandard` — the step can never dead-end on a loc we failed to raise |
