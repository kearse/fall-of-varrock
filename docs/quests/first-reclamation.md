...
| Establish the Southern Watch: **plant the standard** | The Castle Wars **Saradomin Standard** (option **Capture** — a banner on its stand) spawned at 3227,3372, north of the centre stone. The loc is **resolved at boot** from `SouthernWatch.STANDARD_CANDIDATES`, not hard-coded: a varbit-gated candidate is followed through its `transforms` to a concrete id, because the gated parent renders as nothing and cannot be clicked | `onObjOption` (bound defensively from the cache actions of the resolved loc) | `SouthernWatchPlugin.resolveStandard` → `spawnStandard`/`bindStandard` → `SouthernWatch.onCapture` hook → `FirstReclamation.raiseStandard` → `satisfy(establish)` |
