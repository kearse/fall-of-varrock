...
| **4698** | First March (Main Story Quest 2), the same generic framework packing — `QuestEngine.publish` |
...
| `QuestEngine` | begin / advance / advanceTo / satisfy / complete / reset; kill counting; the poll (areas, items, predicates); completion sets the `quest.<key>.done` flag; generic varp publish. |
| `QuestLoginBrief` | The single login reminder: one line per live chain (framework and legacy), three at most, the rest collapsed into a `::quests` pointer. Armed by the login hook, printed on the next poll so every chain has resumed first; a quest that just announced itself is skipped. |
