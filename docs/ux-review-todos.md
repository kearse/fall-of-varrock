...
- [x] **Store buttons render with no provider configured** — every purchase 503s after the click
...
- [x] **Teleport portal is undiscoverable.** No NPC or message ever mentions it; it silently
...
- [x] **Seven skilling teleports land on the identical tile** `(3243, 3193)`
  (`TeleportRegistry.kt:58-65`). Differentiate or collapse into one "Skilling camp" entry.
- [x] **Login screen "Register?" / "Forgot?" links draw but aren't clickable**
...
- [x] **Half-finished rebrand in client plugin list** — mixed "Lof …" and "Kingdom of
  Lumbridge …" descriptors (alerts, pkstats, announcements, cwtimer plugins).
- [x] **Store advertises Discord roles but never says linking is required** (`store.ts:71-100`;
  success page `store/success/page.tsx:25-28` doesn't mention `/link`).
- [x] **Login/register always redirect to home** (`AuthForm.tsx:34`) — buyers lose their place
  in the store; support a `?next=` return URL.

## Lower priority / polish

- [ ] Teleport modal fixed 560×400 overflows right edge below 560px canvas width
  (`LofTeleportsOverlay.java:46-47,105`); login layout `ox` unclamped ~~(fixed: now clamped like `oy`)~~, panel renders off-screen
...
- [x] Supply hand-ins don't show realm-meter movement unless it crosses a threshold
  (`RealmSupply.kt:28-32`) — echo the meter on each deposit.
- [x] Discord event feed marks events posted even when `channel.send()` throws
