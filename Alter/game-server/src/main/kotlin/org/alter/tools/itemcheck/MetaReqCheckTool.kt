 ...
 * skillReqs for the given item ids. Distinguishes "cache params fine but boot-time population broken"
 * from "population fine, enforcement elsewhere broken" when equip level requirements misbehave.
 *
...
fun main(args: Array<String>) {
    ...
    for (id in args.mapNotNull { it.toIntOrNull() }) {
        ...
        println("$id ${d.name}: skillReqs=[$reqs]")
    }
}
