...
object PawnPathAction {
    ...
    private suspend fun walk(it: QueueTask, pawn: Pawn, other: Pawn, opt: Int, lineOfSightRange: Int?) {
        ...
        if (pawn is Player) {
            ...
            if (other is Player) {
                // `opt` is the 0-based player-option slot index (see OpPlayerHandler) — index directly.
                val option = pawn.options.getOrNull(opt)
                if (option != null) {
            ...
            // If the interaction just started combat (e.g. the "Attack" option), keep facing the
            // target: the combat loop requires FACING_PAWN_ATTR == target and clearing it here would
            // instantly cancel the attack (the bug where clicking Attack walked to the target but
            // never hit it; only auto-retaliate worked).
            if (pawn.attr[COMBAT_TARGET_FOCUS_ATTR]?.get() == null) {
                pawn.resetFacePawn()
