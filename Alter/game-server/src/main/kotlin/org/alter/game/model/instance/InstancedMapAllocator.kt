...
class InstancedMapAllocator {
        ...
        for (height in 0 until heights) {
            for (x in 0 until bounds) {
                for (z in 0 until bounds) {
                    ...
                    // Allocate the collision zone EXPLICITLY (mirrors DefinitionSet's region
                    // load): rsmod reads unallocated zones as -1 (fully blocked), so any open
                    ...
                    if (chunk != null) {
                        ...
                        // Filter source entities by the chunk's SOURCE plane (chunk.height), not the
                        // destination plane (chunkH): a chunk set may copy plane N into plane 0 (the
                        // "floors side-by-side" instance layout), and for plain same-plane copies the
                        // two are equal anyway. (Tile.fromRotatedHash can't be used for this — it reads
                        // height from bits 28-29 while toRotatedInteger packs it at 24-25, so its
                        // height is always 0.)
                        copyChunk.getEntities<StaticObject>(EntityType.STATIC_OBJECT).forEach { obj ->
                            if (obj.tile.height == chunk.height && obj.tile.isInSameChunk(copyTile)) {
                                val def = obj.getDef()
                        ...
                        copyChunk.blockedTiles.forEach { tile ->
                            if (tile.height == chunk.height && tile.isInSameChunk(copyTile)) {
                                val localX = tile.x % 8
                                val localZ = tile.z % 8
    ...
    companion object {
         ...
         *
         * The Y-axis starts at 64, NOT 0: the client build area spans the player's zone ±6
         * (±48 tiles), so a player standing inside an instance allocated on the world's bottom
         * row would put the build-area origin at a NEGATIVE zone — rsprot's BuildArea rejects
         * that ("ZoneZ must be in range of 0..<2048"), which aborts the player's whole sync
         * cycle every tick (client never gets the rebuild → void map/desync). Keeping one
         * region row of margin costs ~50 instance slots out of ~5000.
         */
        private val VALID_AREA = Area(bottomLeftX = 6400, bottomLeftY = 64, topRightX = 9600, topRightY = 6400)
