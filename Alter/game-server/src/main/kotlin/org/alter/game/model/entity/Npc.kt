...
class Npc private constructor(val id: Int, world: World, val spawnTile: Tile) : Pawn(world) {
    ...
    /**
     * Release this npc's network avatar back to the pool. Must be called when the
     * npc is permanently removed from the world (see [World.remove]); otherwise the
     * avatar index leaks and a later spawn that reuses the index throws
     * "NPC Avatar with index N is already allocated!".
     */
    internal fun releaseAvatar() {
        if (this::avatar.isInitialized) {
            world.network.npcAvatarFactory.release(avatar)
        }
    }
    ...
    /**
     * When true, this npc blocks (and is blocked by) other such npcs when moving,
     * forming a "wall" (used by the war/battle NPCs). Players are unaffected.
     * See [org.alter.game.model.World.npcClipTiles] / `canTraverse`.
     */
    var collidesWithNpcs = false
