...
object ObjectPathAction {
    ...
    /**
     * Generic door swing used as a fallback for any unconfigured door/gate.
     * Mirrors the game-api World.openDoor/closeDoor convention: the opened object
     * id is the closed id + 1, the rotation steps by one, and the tile shifts
     * according to the wall's facing.
     */
    private fun swingDoor(
        world: World,
        obj: GameObject,
        opening: Boolean,
    ) {
        val oldRot = obj.rot
        val newRot = abs((oldRot + (if (opening) 1 else -1)) and 0x3)
        ...
        val newTile =
            if (opening) {
                when (oldRot) {
                    ...
                    else -> if (diagonal) obj.tile.transform(-1, 0) else obj.tile.transform(0, -1)
                }
            } else {
                when (oldRot) {
                    0 -> if (diagonal) obj.tile.transform(1, 0) else obj.tile.transform(0, 1)
                    1 -> if (diagonal) obj.tile.transform(0, -1) else obj.tile.transform(1, 0)
                    2 -> if (diagonal) obj.tile.transform(-1, 0) else obj.tile.transform(0, -1)
                    else -> if (diagonal) obj.tile.transform(0, 1) else obj.tile.transform(-1, 0)
                }
            }
        val newDoor = DynamicObject(id = obj.id + (if (opening) 1 else -1), type = obj.type, rot = newRot, tile = newTile)
        world.remove(obj)
        world.spawn(newDoor)
    }
    ...
    val objectInteractPlugin: Plugin.() -> Unit = {
        ...
        walk(player, obj, lineOfSightRange) {
            if (!player.world.plugins.executeObject(player, obj.getTransform(player), opt!!)) {
                /*
                 * Generic door fallback: if no specific plugin handled the object and the
                 * clicked action on a wall-type object is "Open"/"Close", swing it using the
                 * standard convention (opened object id = closed id + 1). This makes every
                 * unconfigured door/gate in the world usable without a per-door entry.
                 */
                val action = obj.getDef().actions.getOrNull(opt!! - 1)?.lowercase()
                val isWall = obj.type == 0 || obj.type == WALL_DIAGONAL
                if (isWall && action == "open") {
                    swingDoor(player.world, obj, opening = true)
                } else if (isWall && action == "close") {
