...
object EquipAction {
    ...
    /**
     * True if [p] meets every skill-level requirement to wear [itemId] — the SAME requirement check
     * [equip] applies to real players (ALL skills, base/unboosted levels). Pure (no messages or side
     * effects) so callers that don't perform a full [equip] — e.g. companions equipping from a bank —
     * can enforce identical gear-level guardrails. Returns true when the item has no requirements.
     */
    fun meetsLevelRequirements(
        p: Player,
        itemId: Int,
    ): Boolean {
        val reqs = getItem(itemId).skillReqs ?: return true
        for (entry in reqs.entries) {
            if (p.getSkills().getBaseLevel(entry.key.toInt()) < entry.value) {
                return false
            }
        }
        return true
