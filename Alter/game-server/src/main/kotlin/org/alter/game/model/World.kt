...
class World(val gameContext: GameContext, val devContext: DevContext) {
    ...
    fun canTraverse(
        ...
        // Battle NPCs block one another (forms a wall on the battlefield); players
        // are not Npc so they pass straight through.
        if (pawn is Npc && pawn.collidesWithNpcs &&
            npcClipTiles.contains(npcTileKey(nextTile.x, nextTile.z, nextTile.height))
        ) {
            return false
        }
    ...
    /**
     * Tiles currently occupied by NPCs that block other NPCs (the battlefield
     * "wall"). Rebuilt every tick by NpcCollisionPlugin; read by [canTraverse].
     * Players ignore it (they aren't [Npc]).
     */
    val npcClipTiles = it.unimi.dsi.fastutil.longs.LongOpenHashSet()

    fun npcTileKey(x: Int, z: Int, height: Int): Long =
        (height.toLong() shl 28) or (x.toLong() shl 14) or z.toLong()
    ...
    internal fun cycle() {
        ...
        timersCopy.forEach { key, time ->
            if (time <= 0) {
                try {
                    ...
                    logger.error(e) { "Error executing world timer $key." }
                }
                if (!timers.has(key)) {
                    timers.remove(key)
                }
        ...
        while (groundItemIterator.hasNext()) {
            ...
            val publicDelay = if (groundItem.publicDelayOverride >= 0) groundItem.publicDelayOverride else gameContext.gItemPublicDelay
            val despawnDelay = if (groundItem.despawnDelayOverride >= 0) groundItem.despawnDelayOverride else gameContext.gItemDespawnDelay

            if (groundItem.isPublic() && groundItem.currentCycle >= despawnDelay && groundItem.respawnCycles == -1) {
                /*
            ...
            } else if (!groundItem.isPublic() && groundItem.currentCycle >= publicDelay) {
                /*
                ...
                groundItem.timeUntilDespawn -= publicDelay
    ...
    fun sendExamine(
        ...
        if (examine != null) {
            val extension = if (p.debugExamines) " ($id)" else ""
