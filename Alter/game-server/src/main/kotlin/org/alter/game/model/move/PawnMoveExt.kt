...
/**
 * Queue a straight walk toward [tile] **without invoking the route-finder** — the movement queue
 * decomposes it into 1-tile steps and validates each against collision in [MovementQueue.cycle] as
 * it goes (an illegal step self-clears). For cheap NPC wandering to an adjacent/near tile, where a
 * full [walkTo] route-search is wasted work — and, multiplied across hundreds of roaming mobs, was
 * the dominant per-npc tick cost. Caller should ensure the first step is legal (e.g. via
