...
class PluginRepository(
    ...
    /**
     * Plugins whose constructor threw during loading and were therefore silently
     * dropped. Each entry is (plugin class name -> root-cause message). Collected
     * here so the boot sequence can print a loud, actionable summary instead of a
     * single stdout println per failure that scrolls away unnoticed.
     */
    private val failedPlugins = mutableListOf<Pair<String, String>>()
            ...
            ClassGraph().enableAllInfo().scan().use { result ->
                ...
                plugins.forEach { p ->
                    try {
                        ...
                        // Constructor (and therefore the plugin's init{} block) threw. The most
                        // common cause is onObjOption/onNpcOption's check() failing because the
                        // cache object/npc lacks the expected action verb. Record the root cause
                        // so reportFailures() can surface exactly which plugin died and why.
                        val root = generateSequence(e as Throwable) { it.cause }.last()
                        val reason = "${root::class.java.simpleName}: ${root.message ?: "(no message)"}"
                        failedPlugins += p.name to reason
                        println("Failed to load: ${p.name} plugin -> $reason")
                    }
                }
            }
        } catch (e: Exception) {
    ...
    /**
     * Number of plugins that failed to load (constructor threw). See [reportFailures].
     */
    fun getFailedPluginCount(): Int = failedPlugins.size

    /**
     * Print a loud, actionable summary of every plugin that failed to load. A
     * failed plugin is dropped *entirely* — all of its object/npc/button bindings
     * are silently lost — so this is the single most important boot diagnostic for
     * tracking down "content X does nothing in-game". Called once from the boot
    ...
    fun reportFailures() {
        ...
        sb.appendLine()
        sb.appendLine("================ PLUGIN LOAD FAILURES (${failedPlugins.size}) ================")
        sb.appendLine("These plugins threw during load and were DROPPED — every binding in")
        sb.appendLine("them (object options, npc options, buttons, spawns) is dead in-game.")
        sb.appendLine("Most common cause: an object/npc id whose cache definition lacks the")
        sb.appendLine("expected action verb (fix the id, or use the verb the cache actually has).")
        sb.appendLine("------------------------------------------------------------------")
        failedPlugins.sortedBy { it.first }.forEach { (name, reason) ->
            sb.appendLine("  ✗ ${name.substringAfterLast('.')}")
    ...
    /**
     * Whether [npcId] already has a dedicated per-npc death or full-death handler bound
     * (i.e. a boss/content plugin that spawns its own loot). The generic real-OSRS drop
     * handler uses this to avoid double-dropping on top of bespoke boss tables.
     */
    fun hasNpcDeathHandler(npcId: Int): Boolean =
        npcDeathPlugins.containsKey(npcId) || npcFullDeathPlugins.containsKey(npcId)
