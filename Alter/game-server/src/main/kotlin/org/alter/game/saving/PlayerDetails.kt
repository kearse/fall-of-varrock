...
object PlayerDetails {
    ...
    fun registerAccount(client: Client): Boolean {
        // Display name keeps the as-typed casing; the login key is already normalized.
        val displayName = DisplayName(client.username)
        displayNames[client.loginUsername.lowercase()] = displayName
        val doc = displayName.asDocument().apply {
            append("loginUsername", client.loginUsername)
            if (client.hasPasswordHash()) append("passwordHash", client.passwordHash)
            append("roles", listOf("player"))
            append("donorPoints", 0)
            append("membership", Document("tier", null).append("expires", null))
            append("createdAt", System.currentTimeMillis())
            append("source", "game")
        }
        serialization.saveDocument(client, doc)
        return true
    }
     ...
     */
    fun getAccountDocument(client: Client): Document? {
        return try {
            if (!serialization.playerExists(client)) null else serialization.parseDocument(client)
        } catch (e: Exception) {
            logger.warn(e) { "Failed to read account document for ${client.loginUsername}" }
            null
        }
    }
