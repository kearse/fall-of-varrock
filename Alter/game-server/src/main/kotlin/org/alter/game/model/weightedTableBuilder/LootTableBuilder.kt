...
fun roll(p: Player, lootTables: Set<LootTable>?):Set<GroundItem>  {
    ...
    // Daily War Effort drop bonus (master design brief §3A): a [DROP_BONUS_PERCENT_ATTR]% chance at an
    // extra MAIN-table roll. The percent is set by the WarEffortBonus tier; 0 (default) = no effect.
    val dropBonus = p.attr[org.alter.game.model.attr.DROP_BONUS_PERCENT_ATTR] ?: 0
    if (dropBonus > 0 && p.world.random(99) < dropBonus) {
        tables[TableType.MAIN]?.firstOrNull()?.mainRoll()?.let {
            it.handleToItem(p).forEach { dropSet.add(it) }
        }
    }
