...
abstract class KotlinPlugin(
    ...
    /**
     * Whether [npcId] already has a dedicated per-npc death handler bound (a boss/content
     * plugin that spawns its own loot). Used by the generic real-OSRS drop handler to skip
     * npcs whose loot is handled elsewhere.
     */
    fun hasNpcDeathHandler(npcId: Int): Boolean = r.hasNpcDeathHandler(npcId)
