...
// Run vs the nodebb DB, then restart NodeBB.
// Web app base URL. scripts/apply-nodebb.ps1 substitutes __SITE_URL__ from
// NEXT_PUBLIC_SITE_URL; falls back to localhost when run directly.
...
"(function(){",
"  var SITE='" + SITE + "';",
"  var NAV=[['Home',SITE+'/'],['News',SITE+'/news'],['Hiscores',SITE+'/hiscores'],['Forum','/'],['Guides','/category/13'],['Store',SITE+'/store'],['Vote',SITE+'/vote'],['Play',SITE+'/play']];",
...
"    for(var i=0;i<NAV.length;i++){var on=(NAV[i][0]==='Forum'&&(p==='/'||p.indexOf('/category')===0||p.indexOf('/topic')===0))||(NAV[i][0]==='Guides'&&p.indexOf('/category/13')===0);left+='<a'+(on?' class=\"on\"':'')+' href=\"'+NAV[i][1]+'\">'+NAV[i][0]+'</a>';}",
"    left+='</nav>';",
