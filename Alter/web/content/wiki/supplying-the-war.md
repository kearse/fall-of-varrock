---
title: Supplying the war - the Quartermaster
category: Skilling
summary: What the Quartermaster pays for supplies, how the sell-only Supply Depot works, how Supply Drives double your War Effort, and the difference between War Effort and Realm Supplies.
order: 2
updated: 2026-09-02
---

The **Quartermaster** - at the Lumbridge shop hub and in the Mire crypt - is the end of every production chain: hand him supplies and he consumes the items and credits two things at once - your personal **War Effort** (your lifetime service record) and the kingdom's shared **Realm Supplies** stockpile (`::supply` to check it).

## War Effort vs Realm Supplies

| | War Effort | Realm Supplies |
| --- | --- | --- |
| Whose | **yours** - a personal, lifetime record | **the kingdom's** - one shared stockpile |
| Goes up when | wars, contracts, hand-ins, rogue hunting, kingdom work | skillers hand supplies to a Quartermaster |
| Goes down when | **never** - it is your standing, not a purse | a Minister launches a campaign (1,500) or a King a conquest (2,800) |
| Used for | rank eligibility, the daily bonus, contribution checks (`::service`) | letting the commanders launch the biggest operations |

Scheduled marches, Grand Marches and Lord operations never touch Realm Supplies - the war keeps moving whatever the stockpile says.

## The Supply Depot

Talk to him and pick **Hand in war supplies** (or type `::depot` at his post) to open the **Supply Depot**. It's an ordinary shop window with a tab down the side for each category - Food, Potions, Bars, Ores, Logs, Raw fish - but it's **sell-only**: nothing on the shelf is for sale. The shelf is the catalogue of what the war takes, and the number on each item is the War Effort it pays.

Two ways to hand in:

- **Right-click an item in your inventory** and Sell 1 / 5 / 10 / 50, like any shop.
- **Right-click an item on the shelf** and Hand in 1 / 10 / All - it takes that item straight out of your inventory.

The depot opens on whichever category is boosted by today's Supply Drive.

## What he pays

**Cooked food** (War Effort each)

| Food | WE | Food | WE |
| --- | --- | --- | --- |
| Shrimps / bread / chicken / meat | 1 | Lobster | 4 |
| Herring / trout | 2 | Bass / swordfish | 5 |
| Pike / salmon / tuna | 3 | Monkfish | 6 |
| | | Shark | 8 |

**Finished potions** (4-dose values; lower doses pay less)

| Potion | WE |
| --- | --- |
| Attack / Strength / Defence | 3 |
| Super variants, Prayer, Ranging | 5 |
| Saradomin brew | 8 |
| Super combat | 10 |

**Smithed bars**

| Bar | WE | Bar | WE |
| --- | --- | --- | --- |
| Bronze | 1 | Mithril | 5 |
| Iron | 2 | Adamantite | 8 |
| Steel | 3 | Runite | 12 |

**Raw materials** pay less than the finished good they make - ores below bars, raw fish below cooked - so processing is always worth the extra step.

| Ores | WE | Logs | WE | Raw fish | WE |
| --- | --- | --- | --- | --- | --- |
| Copper / tin / iron | 1 | Logs / oak | 1 | Shrimps to salmon | 1 |
| Silver / coal | 2 | Willow / teak | 2 | Tuna / lobster / bass | 2 |
| Gold / mithril | 3 | Maple | 3 | Swordfish / monkfish | 3 |
| Adamantite | 5 | Mahogany | 4 | Shark | 4 |
| Runite | 8 | Yew 5 / magic 8 | | | |

## Supply Drives - 2× windows

At any given time one category - food, potions, bars, ores, logs or raw fish - is in high demand and pays **double War Effort**. The category rotates on a timer, and you'll see "Supply Drive bonus!" when your deposit catches the window. Smart skillers bank goods ahead and dump them when their category comes up.

## What War Effort gets you

War Effort is your personal, lifetime service record - it feeds rank eligibility and daily bonuses, and it is never spent. Between the Quartermaster, [resource contracts](/wiki/slayer-and-contracts) and the marches, a dedicated skiller can climb the ladder without ever winning a duel. `::service` shows your full ledger: wars fought and won, supplies handed in, lifetime War Effort.

Earning War Effort through the day also builds a **daily bonus** to your XP rate and drop rate - it tiers up the more you contribute and resets each day. Check your current tier and progress with `::warbonus`.
