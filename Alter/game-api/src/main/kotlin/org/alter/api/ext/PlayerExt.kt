...
/**
 * Draw the native OSRS hint arrow (the floating yellow/red marker) over a world tile.
 * Off-screen it becomes a directional edge arrow, so it guides the player toward [x],[z]
 * from any distance — the "go here" arrow used for quest/objective guidance. Clear it with
 * [clearHintArrow].
 */
fun Player.setTileHintArrow(
    x: Int,
    z: Int,
    height: Int = 0,
) {
    write(HintArrow(HintArrow.TileHintArrow(x, z, height, HintArrow.TileHintArrow.HintArrowTilePosition.CENTER)))
}

/** Draw the hint arrow over an npc (tracks the npc; only renders while it's in view). */
fun Player.setNpcHintArrow(npcIndex: Int) {
...
fun Player.hasOption(
    ...
    return if (id != -1) options.getOrNull(id - 1) == option else options.any { it == option }
}
