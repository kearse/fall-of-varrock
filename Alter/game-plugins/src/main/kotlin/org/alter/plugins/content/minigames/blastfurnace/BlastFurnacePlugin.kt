 ...
 * (unless a [BlastFurnace.feeWaivers] predicate says Keldagrim is paying).
 *
 * The Foreman's Talk-to goes through the shared [NpcTalk] router (`bindTalk`), so quests can
 * claim his conversation on their own steps (The Guns of Asgarnia does) without editing this file;
 * his everyday explanation is the default branch. His **Pay** option is the coffer.
 */
class BlastFurnacePlugin(
    ...
    private class Machine(val owner: Player) {
        ...
        fun readyCount() = ready.values.sum()
    }

    private val machines = HashMap<Player, Machine>()

    init {
        BlastFurnace.oreInMachine = { p -> machines[p]?.let { it.oreCount() + it.readyCount() } ?: 0 }
        ...
        onWorldInit {
            runCatching {
                ...
                // Hand-placed here (world-spawn dedupe skips the data row within 6 tiles) so he never wanders.
                val foreman = Npc(getRSCM(BlastFurnace.FOREMAN_KEY), world.snapToWalkable(BlastFurnace.FOREMAN_TILE, maxRadius = 2), world)
        ...
        // The dispenser is a multi-loc (9092, varbit 936): the click arrives on the CHILD id, so bind the children.
        runCatching { onObjOption(obj = BlastFurnace.DISPENSER_EMPTY_KEY, option = "check") { status(player) } }
            .onFailure { logger.warn { "blast-furnace: empty-dispenser bind failed: ${it.message}" } }
        BlastFurnace.DISPENSER_READY_KEYS.forEach { key ->
            runCatching { onObjOption(obj = key, option = "take") { takeBars(player) } }
                .onFailure { logger.warn { "blast-furnace: $key Take bind failed: ${it.message}" } }
            runCatching { onObjOption(obj = key, option = "check") { status(player) } }
                .onFailure { logger.warn { "blast-furnace: $key Check bind failed: ${it.message}" } }
        }
        ...
        runCatching { onObjOption(obj = BlastFurnace.PUMP_KEY, option = "operate") { player.message("Pumpy has the pump. The dwarves run the machine.") } }
        runCatching { onObjOption(obj = BlastFurnace.PEDALS_KEY, option = "pedal") { player.message("Thumpy's on the pedals. The dwarves run the machine.") } }
        runCatching { onObjOption(obj = BlastFurnace.GAUGE_KEY, option = "read") { player.message("The furnace is at working temperature — the dwarves keep it there.") } }
        ...
        // The Foreman: Talk-to through the shared router (quests attach their own branches), Pay = the coffer.
        if (bindTalk(BlastFurnace.FOREMAN_KEY)) {
            NpcTalk.register(BlastFurnace.FOREMAN_KEY, NpcTalk.PRIORITY_DEFAULT) { _ -> { p -> foreman(p) } }
        }
        runCatching { onNpcOption(npc = BlastFurnace.FOREMAN_KEY, option = "pay") { player.queue { coffer(player) } } }
            .onFailure { logger.warn { "blast-furnace: Foreman Pay bind failed: ${it.message}" } }
        ...
        listOf(BlastFurnace.COAL_BAG, BlastFurnace.OPEN_COAL_BAG).forEach { bag ->
            ...
            }.onFailure { logger.warn { "blast-furnace: coal bag '$bag' option bind failed: ${it.message}" } }
        }
        runCatching { onItemOption(item = BlastFurnace.COAL_BAG, option = "open") { swapBag(player, BlastFurnace.COAL_BAG, BlastFurnace.OPEN_COAL_BAG) } }
        runCatching { onItemOption(item = BlastFurnace.OPEN_COAL_BAG, option = "close") { swapBag(player, BlastFurnace.OPEN_COAL_BAG, BlastFurnace.COAL_BAG) } }
    ...
    private suspend fun QueueTask.putOre(p: Player) {
        val waived = BlastFurnace.feeWaived(p)
        val coffer = p.attr[BlastFurnace.COFFER] ?: 0
        if (!waived && coffer < BlastFurnace.COFFER_PER_MINUTE) { p.message("The coffer is empty — deposit at least ${BlastFurnace.COFFER_MIN_DEPOSIT} coins (Use the coffer or Pay the Foreman) before the dwarves will run your ore."); return }
        ...
        if (waived) p.message("Keldagrim is running this order at its own expense — the coffer is not drawn.")
    ...
    private fun takeBars(p: Player) {
        ...
        var taken = 0
        var xp = 0.0
        val takenByBar = LinkedHashMap<BlastFurnace.Bar, Int>()
        ...
        takenByBar.forEach { (bar, n) -> BlastFurnace.notifyBars(p, bar, n) }
    }
    ...
    private suspend fun QueueTask.coffer(p: Player) {
        ...
        chatNpc(p, "The coffer holds ${have} coins of yours. The furnace draws ${BlastFurnace.COFFER_PER_MINUTE} a minute<br>while your ore is in the machine (${BlastFurnace.COFFER_PER_MINUTE * 60} an hour). Minimum deposit ${BlastFurnace.COFFER_MIN_DEPOSIT}.", npc = getRSCM(BlastFurnace.FOREMAN_KEY), title = BlastFurnace.FOREMAN_NAME)
    ...
    /** The Foreman's everyday explanation — the default [NpcTalk] branch (quests claim him above this). */
    private suspend fun QueueTask.foreman(p: Player) {
        val id = getRSCM(BlastFurnace.FOREMAN_KEY)
        chatNpc(p, "Welcome to the Blast Furnace. My dwarves run the machine —<br>you run the coffer: ${BlastFurnace.COFFER_PER_MINUTE * 60} coins an hour while your ore is in.<br>Ore on the belt, bars from the dispenser. Mind the heat.", npc = id, title = BlastFurnace.FOREMAN_NAME)
        chatNpc(p, "Half the coal of a normal furnace: steel 1, mithril 2, adamant 3, rune 4.<br>${BlastFurnace.MAX_ORE} ores and ${BlastFurnace.MAX_COAL} coal in the machine at once, ${BlastFurnace.MAX_BARS_PER_TYPE} bars<br>of a kind in the dispenser. Ice gloves or a bucket of water for the bars.", npc = id, title = BlastFurnace.FOREMAN_NAME)
    }
    ...
    private fun status(p: Player) {
        ...
        val fee = if (BlastFurnace.feeWaived(p)) "Keldagrim's account" else "${p.attr[BlastFurnace.COFFER] ?: 0} coins"
        p.message("<col=0000ff>Blast Furnace:</col> coffer $fee | on the belt: $queued | coal in machine ${m?.coal ?: 0} | bars ready: $ready | coal bag ${p.attr[BlastFurnace.COAL_IN_BAG] ?: 0} | bars made ${p.attr[BlastFurnace.BARS_MADE] ?: 0}.")
    }
}
