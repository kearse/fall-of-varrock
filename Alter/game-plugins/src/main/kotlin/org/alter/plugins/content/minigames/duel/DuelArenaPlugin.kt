 ...
 * exact tiles they were challenged on.
 *
 ...
 *
 * Every duel is a pure 1v1: both fighters' companions are parked for its duration ([CompanionPolicy]
 * — companions are PvE-only in human PvP) and the old "Allow companions" rule stays locked off
 * ([DuelArena.companionsOpen]).
 */
class DuelArenaPlugin(
    ...
    init {
        ...
        onLogin {
            ...
            player.sendOption(CHALLENGE_OPTION, CHALLENGE_OPTION_SLOT)
        ...
        onPlayerOption(CHALLENGE_OPTION) {
            // OrNull: the target can log off between the click and the walk-up completing — the
            // hard deref would NPE the whole cycle (see TradingPlugin's "Trade with" handler).
            val target = player.getInteractingPlayerOrNull() ?: return@onPlayerOption
            challenge(player, target)
        }

        // Pure 1v1: while a player is in a duel (countdown or fight) their companions are parked —
        // benched in place by CompanionRegistry before any brain runs, refused summon/recruit, and
        // re-summoned beside the owner the tick after the duel resolves. Without this the brain's
        // formation snap would teleport them into the pit the tick after begin() moves the owner.
        CompanionPolicy.register { owner, _ ->
            if (DuelArena.duelOf(owner) != null) CompanionPolicy.Verdict("a duel is fought alone") else null
        }
        ...
        onCommand("duelpittest", Privilege.ADMIN_POWER, description = "Teleport into a test copy of the obstacle pit") {
            val instance = RaidInstance.allocate(world, OBSTACLE_SOURCE, exitTile = world.gameContext.home, owner = player.uid, autoDeallocate = true)
            if (instance == null) {
    ...
    private fun challenge(player: Player, target: Player) {
        ...
        // Bots and companions (a Companion IS a PkBot) can't be staked against — players only.
        ...
        // No arranging duels from inside other content's instances (Wizard Tower, boss lairs...) —
    ...
    private suspend fun QueueTask.pickRulesThenStake(player: Player, target: Player) {
        ...
        val rules = DuelRules(
            ...
            allowCompanions = false, // pure 1v1 — see DuelArena.companionsOpen
    ...
    private fun begin(a: Player, stakeA: List<Item>, b: Player, stakeB: List<Item>, rules: DuelRules) {
        ...
        val instance = RaidInstance.allocate(world, source, exitTile = world.gameContext.home, owner = a.uid, autoDeallocate = false)
        if (instance == null) {
    ...
    private fun tick(d: Duel) {
        if (d.a.index < 0 || d.b.index < 0) {
            // A leaver's duel is resolved by onLogout before they're unregistered, so this is the
            // one path that skips the logout hooks (a stale-registration reclaim). Don't strand the
            // survivor in the pit with no draw timer: call it a draw. Each side gets their OWN stake
            // back — the vanished side's via the login crash-refund (their escrow blob stays put) —
            // so nothing can ever be paid twice, which a forfeit-to-the-survivor could (the leaver's
            // last save may still carry the escrow).
            val gone = if (d.a.index < 0) d.a else d.b
            logger.warn { "duel principal ${gone.username} vanished without a logout hook — resolving as a draw" }
            resolveDraw(d, "${gone.username} vanished", DuelLog.DRAW_VANISHED)
            return
        }
    ...
    companion object {
        /** The player right-click label. The client's chat-request relabel (lofchatrequests) keys
         *  on the SLOT, not this text, so renaming it is a one-line server change. */
        const val CHALLENGE_OPTION = "Challenge"

        /** Player right-click slot for [CHALLENGE_OPTION] (2..5 = Attack/Follow/Trade/Report; 6 is free). */
        ...
        // The instance's exit tile is the world HOME (see begin()). Only a fallback — resolve()
        // sends both fighters back to their return tiles; the exit catches the odd path (the
        // death-respawn hop, logout placement, allocator teardown sweep). NOT the old arena lobby:
        // under the Fallen Varrock zoning the Al Kharid grounds sit inside the main wilderness box.
