...
object QuestBook {
    ...
    // Asgarnia's finale, takes 19 when it lands).
    ...
    // Main Story Quest 2, built after the quests around it: slots are build order, not story order.
    const val FIRST_MARCH = 18      // First March (Main Story Quest 2)

    /** The highest chain index the client journal lists (`LofQuest.CHAIN.size() - 1`). */
    const val LAST_INDEX = FIRST_MARCH
