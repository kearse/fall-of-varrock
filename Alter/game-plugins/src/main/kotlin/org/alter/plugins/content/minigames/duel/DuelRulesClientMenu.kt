...
object DuelRulesClientMenu {
    ...
    /** Index of the Allow-companions toggle — refused while [DuelArena.companionsOpen] is false. */
    private const val R_ALLOW_COMPANIONS = 11
    ...
    fun toggle(p: Player, index: Int) {
        ...
        if (index == R_ALLOW_COMPANIONS && !s.rules[index] && !DuelArena.companionsOpen) {
            p.message("Companions can't fight in duels yet.")
            return
        }
    ...
    private fun unpackInto(packed: Int, rules: BooleanArray, slots: BooleanArray) {
        ...
        // A preset saved while the 4v4 was open can't re-arm it (validate() in accept() is the belt).
        if (!DuelArena.companionsOpen) rules[R_ALLOW_COMPANIONS] = false
    }
