...
class GotrPlugin(
    ...
    private fun placeCell(p: Player) {
        ...
        if (i < 0) {
            // The stock map also carries "Inactive cell tile" locs in a ring around the Great Guardian
            // (r14484 dump: 3608-3622 × 9503-9510); the game's barrier line is the row across the rift's
            // approach (Gotr.CELL_TILES), so a click on a ring tile is a no-op with a pointer, not silence.
            p.message("The barrier line is the row of cell tiles across the rift's approach, north of the Great Guardian.")
            return
        }
        val b = barriers[i]
        if (b.broken) {
