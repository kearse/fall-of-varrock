...
class StallThievingPlugin(
    ...
    private fun rewardPlayer(player: Player, entry: StallEntry, stallName: String) {
        ...
        // Don't silently destroy loot when the inventory is full — drop the un-added
        // remainder at the player's feet (matches chest/pickpocket behaviour).
        val leftover = amount - transaction.completed
        if (leftover > 0) {
            player.world.spawn(
                GroundItem(item = getRSCM(loot.item), amount = leftover, tile = player.tile, owner = player),
            )
        }
