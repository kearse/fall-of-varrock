...
class LootingBagPlugin(
    ...
    init {
        ...
        onGlobalItemPickup {
            if (player.inventory.contains(getRSCM("item.looting_bag_22586"))) {
    ...
    fun store(
        ...
        // Looting bag works everywhere (not just the Wilderness) on this server.
        store(p, item, amount)
    }
    ...
    fun store(
        ...
        // Restored: the old body used a stale 2-arg ItemContainer constructor. The current
        // API takes only the container key (see check()/bank() below).
