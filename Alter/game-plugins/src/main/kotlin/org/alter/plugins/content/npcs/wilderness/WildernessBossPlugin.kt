...
/**
 * Spawns every [WildernessBosses] boss at its lair, registers its combat def from the
 * registry's stats, and handles its **death → loot + Boss points + Collection Log +
 * rare-drop broadcast** (the proven [org.alter.plugins.content.npcs.kbd.KbdBossPlugin]
 * shape, generalised over the roster). The OSRS attack rotations live in
 * [WildernessBossCombatPlugin].
 */
class WildernessBossPlugin(
    ...
    init {
        for (boss in WildernessBosses.all) {
    ...
    private fun dropLoot(npc: Npc, killer: Player, boss: WildernessBosses.WildBoss) {
