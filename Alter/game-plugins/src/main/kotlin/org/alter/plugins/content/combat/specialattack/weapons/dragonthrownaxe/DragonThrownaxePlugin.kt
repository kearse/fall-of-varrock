 ...
 * 25% energy, +25% ACCURACY (damage unchanged) â€” OSRS Wiki, Dragon thrownaxe.
 *
 * NOT `executeInstantly`: that flag routes the spec-orb click through the instant (granite-maul)
 * path, which only fires at a MELEE-ADJACENT target â€” thrownaxes are used from four tiles, so
...
class DragonThrownaxePlugin(
    ...
    init {
        // Both thrownaxe defs: 21207 is a second "Dragon thrownaxe" in this cache and had no
        // special registered against it (player report 2026-09-18).
        for (axe in listOf("item.dragon_thrownaxe", "item.dragon_thrownaxe_21207")) {
            runCatching {
                SpecialAttacks.register(axe, 25) {
                    ...
                    // The thrown axe is the ammo: one leaves the weapon slot per throw (the spec used
                    // to throw for free). Remove the axe actually WIELDED, not a hardcoded id, or the
                    // second def would throw free forever.
                    player.getEquipment(EquipmentType.WEAPON)?.let { worn ->
                        player.equipment.remove(worn.id, 1, beginSlot = EquipmentType.WEAPON.id)
                    }
                }
            }
        }
