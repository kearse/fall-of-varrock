...
class TitlePlugin(
    ...
    init {
        ...
        onCommand("title", description = "Show your rank, armour tier, and next rank cost") {
            ...
            if (t.roster > 0) {
                player.message("Your rank keeps a roster of <col=ffae00>${t.roster}</col> soldier companion${if (t.roster > 1) "s" else ""} (General Zo)${if (t.roster > 1) " — one at your side at a time, swap the rest in from the bench" else ""}.")
            }
