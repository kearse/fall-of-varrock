...
// Idempotent. PROD copy: no hardcoded cids - sub-forums are resolved by walking
// /api/category/<cid> children, and the patch note posts to "Game Updates" by name.
const BASE = process.env.NODEBB_URL || "http://127.0.0.1:4567";
const TOKEN = process.env.NODEBB_TOKEN;
if (!TOKEN) { console.error("NODEBB_TOKEN required"); process.exit(1); }
const UID = 1;
...
async function childrenOf(cid) {
  try { const j = await (await fetch(`${BASE}/api/category/${cid}`)).json(); return j.children || []; }
  catch { return []; }
}
...
async function main() {
  ...
  for (const c of cats) {
    ...
    console.log(`  * ${decode(c.name)} -> ${icon} ${bgColor}`);
    await sleep(120);
  }

  // Sub-forums (3rd level - not returned by /api/categories): resolve by walking
  // the children of every 2nd-level board we know about.
  console.log("Styling sub-forums...");
  let gameUpdatesCid = null;
  for (const parentName of ["Announcements", "Suggestions"]) {
    const parent = cats.find((c) => decode(c.name) === parentName);
    if (!parent) { console.log(`  ! parent "${parentName}" not found`); continue; }
    for (const sub of await childrenOf(parent.cid)) {
      const name = decode(sub.name);
      if (name === "Game Updates") gameUpdatesCid = sub.cid;
      const s = STYLE[name];
      if (!s) { console.log(`  ? no style for sub "${name}"`); continue; }
      const [icon, bgColor] = s;
      try { await api("PUT", `/api/v3/categories/${sub.cid}`, { icon, bgColor, color: "#ffffff" }); console.log(`  * sub ${name} (${sub.cid}) -> ${icon}`); } catch (e) { console.log("  ! " + e.message); }
      await sleep(120);
    }
  }

  // First patch note in Game Updates
  if (gameUpdatesCid) {
    const title = "Patch 1.0 - The Realm Goes Live";
    if (!(await topicTitles(gameUpdatesCid)).has(title)) {
      await api("POST", "/api/v3/topics", { cid: gameUpdatesCid, title, content: PATCH_1_0 });
      console.log("  + posted: " + title);
    } else console.log("  = patch note exists");
  } else console.log("  ! Game Updates not found, patch note skipped");
  console.log("Done.");
}
...
## Progression
- The full **feudal rank ladder** (Peasant to King) with armour gating and coloured names.
...
Jump into the **Guides** board to learn the ropes, and we'll see you on the battlefield.
